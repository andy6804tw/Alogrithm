...
此版本第一個物品都是0(依照索引值取物)，4個物品index為0~4共五個(首相拋棄)。
```java
package dynamic;
...
public class Recursive_Knapsack {
	...
	static int knapsack(int i, int W) {
		...
		if (list.get(i-1).weight > W) {
			return knapsack(i - 1, W);
		} else {
			return c[i][W] = Math.max(knapsack(i - 1, W), knapsack(i - 1, W - list.get(i-1).weight) + list.get(i-1).value);
		}
	}
